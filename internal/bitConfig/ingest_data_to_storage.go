...
func PostFailuresData() {
	...
	if err != nil {
		log.Printf("error reading config_failures dir error: %v", err)
		return
	}
	for i, f := range files {
		...
		if e != nil {
			log.Printf("error close request body file number %v, error: %v", i+1, e)
			return
		}
	}
}
...
func PostGroupFilterData() {
	...
	for i, f := range files {
		...
		e = storageResp.Body.Close()
		if e != nil {
			log.Printf("error close request body %v, error: %v", i+1, e)
			return
		}
	}
}
